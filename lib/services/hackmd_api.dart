...
class HackmdApi {
  ...
  /// Resolves the API `noteId` for a `hackmd.io` note URL. A canonical URL
  /// (`hackmd.io/<id>`) already has the id as its only path segment; a
  /// custom-aliased one (`hackmd.io/@user/alias`, or a team note) doesn't —
  /// the alias isn't the real id, so it has to be found by listing the
  /// account's notes and matching on `permalink`. Returns `null` if no
  /// match is found (e.g. the note belongs to someone else's account).
  static Future<String?> resolveNoteId(String token, Uri hackmdUrl) async {
    ...
    for (final note in notes) {
      if (note.permalink == alias || note.id == alias) return note.id;
    }
    return null;
  }
}
